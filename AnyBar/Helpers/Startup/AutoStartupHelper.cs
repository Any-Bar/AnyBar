...
namespace AnyBar.Helpers.Startup;
...
public class AutoStartupHelper
{
    ...
    private const string LogonTaskName = $"{Constants.AnyBar} Startup";
    private const string LogonTaskDesc = $"Automatically start {Constants.AnyBar} on system startup";
    ...
    private static bool CheckRegistry()
    {
        try
        {
            ...
            if (key != null)
            {
                ...
                var action = (key.GetValue(Constants.AnyBar) as string) ?? string.Empty;
    ...
    {
        using var key = Registry.CurrentUser.OpenSubKey(StartupPath, true);
        key?.DeleteValue(Constants.AnyBar, false);
        return true;
    }
    ...
    {
        using var key = Registry.CurrentUser.OpenSubKey(StartupPath, true);
        key?.SetValue(Constants.AnyBar, $"\"{Constants.ExecutablePath}\"");
        return true;
    }
}
